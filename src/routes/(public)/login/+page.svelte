...
<div class="min-h-screen flex items-center justify-center p-4 sm:p-6 relative bg-baby-pink">
	...
	<!-- Main Card Container -->
	<div class="w-full max-w-4xl bg-white/90 backdrop-blur-md border border-slate-100 rounded-[32px] shadow-2xl overflow-hidden grid md:grid-cols-12 md:h-[600px] relative z-10 animate-fade-in-up">
		
		<!-- Left Side: Branding, Slogan & Ambient Waves (Visible on desktop/tablet) -->
		<div class="hidden md:flex md:col-span-5 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-950 p-8 flex-col justify-between relative overflow-hidden text-white">
			<!-- Soft animated waves/blood cells inside left pane -->
			<div class="absolute inset-0 bg-red-950/10 pointer-events-none z-0"></div>
			<div class="absolute -top-16 -left-16 w-48 h-48 bg-red-500/10 rounded-full blur-3xl"></div>
			<div class="absolute -bottom-16 -right-16 w-48 h-48 bg-red-650/10 rounded-full blur-3xl"></div>
			
			<!-- Top Section: Logo -->
			<div class="relative z-10 flex items-center gap-3">
				<img src="/logo.png" alt="LifeLink Logo" class="h-9 w-9 object-contain filter brightness-110" />
				<span class="text-xl font-black tracking-tight text-white">LifeLink</span>
			</div>

			<!-- Middle Section: Slogan & pulsing heartbeat -->
			<div class="relative z-10 my-auto space-y-6">
				<div class="flex justify-center py-4">
					...
					<div class="relative inline-block animate-pulse">
						...
						<span class="absolute inset-0 flex items-center justify-center text-xs font-bold text-white uppercase tracking-widest mt-0.5">Live</span>
					</div>
				</div>
				<div class="space-y-3 text-center">
					<h3 class="text-lg font-bold leading-snug">Every Drop Counts ❤️</h3>
					<p class="text-xs text-slate-300 max-w-[220px] mx-auto leading-relaxed">
						Connecting donors and patients in real-time to eliminate emergency matching latency.
					</p>
				</div>
			</div>

			<!-- Bottom Section: Info -->
			<div class="relative z-10 text-[10px] text-slate-400 font-semibold tracking-wider text-center">
				LIFELINK PLATFORM © 2026
			</div>
		</div>

		<!-- Right Side: Standard Login Form or System Initial setup (Always visible) -->
		<div class="col-span-12 md:col-span-7 p-6 sm:p-8 flex flex-col justify-center bg-white">
			
			{#if !data.hasAdminAccount}
				<!-- State A: System Initialization Setup Screen -->
				<div class="space-y-6">
					<div class="text-center md:text-left space-y-1">
						<!-- Logo visible on mobile only inside form -->
						<div class="flex md:hidden items-center justify-center gap-2 mb-2">
							<img src="/logo.png" alt="LifeLink Logo" class="h-8 w-8 object-contain" />
							<span class="text-lg font-black text-slate-900 tracking-tight">LifeLink</span>
						</div>
						<h2 class="text-xl font-bold text-red-700">Initialize Admin Profile</h2>
						<p class="text-slate-400 text-xs">No primary administrator account detected. Setup the system profile.</p>
					</div>
					...
					<form onsubmit={handleLogin} class="space-y-4">
						...
						<div class="flex flex-col gap-1">
							...
							<div class="relative w-full">
								...
								<button
									type="button"
									class="absolute inset-y-0 right-0 flex items-center pl-3.5 text-slate-400 hover:text-red-700 transition cursor-pointer"
