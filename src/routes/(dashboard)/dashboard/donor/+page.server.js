...
export async function load({ locals }) {
	...
	const badges = [
		{ name: 'Bronze Donor', desc: 'Completed 1 whole blood donation drive.', progress: donationsCount >= 1 ? 100 : 0, earned: donationsCount >= 1, icon: 'award' },
		{ name: 'Silver Donor', desc: 'Completed 3 whole blood donation drives.', progress: donationsCount >= 3 ? 100 : Math.round((donationsCount / 3) * 100), earned: donationsCount >= 3, icon: 'award' },
		{ name: 'Gold Donor', desc: 'Completed 5 whole blood donation drives.', progress: donationsCount >= 5 ? 100 : Math.round((donationsCount / 5) * 100), earned: donationsCount >= 5, icon: 'award' },
		{ name: 'Life Saver Badge', desc: 'Successfully matched and accepted emergency tickets.', progress: donationsCount >= 1 ? 100 : 0, earned: donationsCount >= 1, icon: 'trophy' }
	];
